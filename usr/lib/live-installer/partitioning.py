...
def getoutput(command):
    return execute(command).stdout.read().strip()
...
class PartitionSetup(gtk.TreeStore):
    def __init__(self, installer):
        ...
        self.disks = self.get_attached_disks()
    ...
    def get_attached_disks(self):
        disks = []
        try:
            # we want to skip the live USB device the installer is run from (mounted on /)
            exclude_device = re.search(r'/([^/]+)[0-9]+$',
                                       os.readlink(execute("awk '/^\/dev[^ ]+ \/ /{print $1}' /proc/mounts").stdout.read().strip())).group(1)
        except AttributeError:
            # if /dev/whatever, mounted to real /, is not a symlink to /dev/sdXN, no biggie
            exclude_device = ''
        lsblk = execute('lsblk -rindo TYPE,NAME,RM,SIZE,MODEL')
        for line in lsblk.stdout:
            type, device, removable, size, model = line.split(" ", 4)
            if type in ("disk", "loop") and device != exclude_device:
                device = "/dev/" + device
    ...
    def __init__(self, partition):
        ...
        try:
            ...
            if getoutput("/sbin/gdisk -l {} | awk '/ EF00 /{{print $1}}'".format(partition.disk.device.path)) == str(partition.number):
                description = self.is_efi_partition(True)
        ...
        if self.is_efi_partition(): self.mount_as = EFI_MOUNT_POINT
        if "swap" in self.type: self.mount_as = SWAP_MOUNT_POINT

    def is_efi_partition(self, set_true=False):
        str = 'EFI System Partition'
        return str if set_true else str == self.description
