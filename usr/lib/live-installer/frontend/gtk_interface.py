...
class Timezone:
    ...
    MAP_CENTER = (373, 261)  # pixel center of where equatorial line and 0th meridian cross on our bg map
    MAP_SIZE = (800, 409)    # size of the map image
    def getPosition(self, lat, lon):
        dx = self.MAP_SIZE[0] / 2 / 180
        dy = self.MAP_SIZE[1] / 2 / 90
        # formulae from http://en.wikipedia.org/wiki/Miller_cylindrical_projection
        x = self.MAP_CENTER[0] + dx * lon
        y = self.MAP_CENTER[1] - dy * math.degrees(5/4 * math.log(math.tan(math.pi/4 + 2/5 * math.radians(lat))))
        return int(x), int(y)
