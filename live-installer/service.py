    ...
    if "live_user" in config.live:
        ...
        if "live_password" in config.live:
            if os.system("which chpasswd &>/dev/null") == 0:
                ...
                fp.write("{}:{}\n".format(config.live["live_user"],config.live["live_password"]))
                fp.close()
                os.system("cat /tmp/.passwd | chpasswd ; rm -f /tmp/.passwd")
            else:
                os.system("echo -e \"{0}\\n{0}\\n\" | passwd {1}".format(config.live["live_password"],config.live["live_user"]))
        if "live_groups" in config.live:
            for i in config.live["live_groups"]:
                os.system("usermod -aG \"{}\" \"{}\"".format(i,config.live["live_user"]))
